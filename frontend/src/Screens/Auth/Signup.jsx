...
// //     // Add a small delay to ensure sessionStorage is updated before navigation
// //     setTimeout(() => {
// //       switch (validRole) {
// //         case 'scorer':
...
// //             <button
// //               type="button"
// //               className={`flex-1 py-2 px-4 text-sm font-medium rounded-l-md ${activeTab === 'viewer'
...
// //             <button
// //               type="button"
// //               className={`flex-1 py-2 px-4 text-sm font-medium ${activeTab === 'scorer'
// //                   ? 'bg-[#16638A] text-white'
...
// //             <button
// //               type="button"
// //               className={`flex-1 py-2 px-4 text-sm font-medium rounded-r-md ${activeTab === 'organiser'
// //                   ? 'bg-[#16638A] text-white'
// //                   : 'bg-white text-gray-700 border border-gray-300'
// //                 }`}
...
//       // Clear any existing role in sessionStorage before setting new one
//       sessionStorage.removeItem('userSignupRole');
...
//       console.log('Creating Google user with role:', activeTab);
//       const createdUser = await createUserInDatabase(userData);
//       console.log('Google user created, redirecting with role:', activeTab);
...
const Signup = () => {
  ...
  const handleEmailSignup = async (e) => {
    ...
    try {
      ...
      // Store role in localStorage (more persistent than sessionStorage)
      localStorage.setItem('userRole', activeTab);
      
      // Redirect immediately without waiting for AuthContext to update
      redirectBasedOnRole(activeTab);
    } catch (error) {
      console.error('Signup error:', error);
      if (error.code === 'auth/email-already-in-use') {
  ...
  const redirectBasedOnRole = (role) => {
    ...
    // Force a hard redirect to ensure proper role handling
    switch (role) {
      case 'scorer':
        window.location.href = '/scorer-home';
        break;
      case 'organiser':
