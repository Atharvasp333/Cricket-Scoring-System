...
//                     time: data.time || '',
//                     venue: data.venue || '',
//                     team1: data.team1 || { name: '', players: [] },
//                     team2: data.team2 || { name: '', players: [] },
    ...
    const submitMatch = async () => {
        try {
            ...
            if (matchData.dateTime) {
                ...
                payload.date = matchData.date;
                payload.time = matchData.time;
            }
            
            // Ensure team1 and team2 are properly formatted
            if (matchData.team1) {
                if (typeof matchData.team1 === 'string') {
                    payload.team1 = { name: matchData.team1, players: match.team1?.players || [] };
                } else {
                    payload.team1 = matchData.team1;
                }
            }
            
            if (matchData.team2) {
                if (typeof matchData.team2 === 'string') {
                    payload.team2 = { name: matchData.team2, players: match.team2?.players || [] };
                } else {
                    payload.team2 = matchData.team2;
                }
            }
            
            // Set teams array for backward compatibility
            payload.teams = [
                typeof payload.team1 === 'object' ? payload.team1.name : payload.team1,
                typeof payload.team2 === 'object' ? payload.team2.name : payload.team2
            ];
            ...
            <div className="bg-white shadow-md rounded-lg overflow-hidden border border-gray-200">
                ...
                <div className="px-4 py-4 border-b border-gray-200 bg-gray-50">
                    <div className="flex items-center justify-center space-x-4">
                        <div className="text-center">
                            <div className="bg-white rounded border border-gray-200 p-3 min-w-[120px]">
                                <h3 className="text-lg font-bold text-gray-800 mb-1">{match.team1?.name || match.teams?.[0] || 'Team 1'}</h3>
                                {matchData.team1?.players?.length > 0 && (
                                    <p className="text-xs text-gray-600 bg-blue-50 px-2 py-0.5 rounded-full inline-block">
                                        {matchData.team1.players.length} players
                                    </p>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="text-center">
                            <div className="bg-white rounded border border-gray-200 p-3 min-w-[120px]">
                                <h3 className="text-lg font-bold text-gray-800 mb-1">{match.team2?.name || match.teams?.[1] || 'Team 2'}</h3>
                                {matchData.team2?.players?.length > 0 && (
                                    <p className="text-xs text-gray-600 bg-blue-50 px-2 py-0.5 rounded-full inline-block">
                                        {matchData.team2.players.length} players
                                    </p>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="bg-white rounded border border-gray-200 p-3">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                            <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded p-3 border border-blue-200 shadow-sm">
                                ...
                                <p className="text-lg font-bold text-blue-900">{matchData.total_overs || 'Not set'}</p>
                            </div>
                            ...
                            <div className="bg-gradient-to-br from-green-50 to-green-100 rounded p-3 border border-green-200 shadow-sm">
                                ...
                                <p className="text-lg font-bold text-green-900">{matchData.powerplay_overs || 'Not set'}</p>
                            </div>
                            ...
                            <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded p-3 border border-purple-200 shadow-sm">
                                ...
                                <p className="text-lg font-bold text-purple-900">
                                    {matchData.drs_enabled ? (matchData.drs_reviews || 'ON') : 'OFF'}
                                </p>
                                <p className="text-xs text-purple-600 mt-0.5">
                                    {matchData.drs_enabled ? 'reviews per team' : 'disabled'}
                                </p>
                            </div>
                        </div>
                    </div>
